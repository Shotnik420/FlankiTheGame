...
func _input(event: InputEvent) -> void:
	if event is InputEventMouseMotion:
		...
		if current_module.has_method("handle_mouse"):
			current_module.handle_mouse(event.relative.x, event.relative.y)
		if !mouse_block:
...
func camera_sipping():
	var camera_pos = camera.rotation_degrees
	var move_tween : Tween = get_tree().create_tween().set_parallel(true).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CUBIC)
	camera.rotation_degrees.x += 5

	move_tween.tween_property(camera, "rotation_degrees:x", camera_pos.x,0.6)
