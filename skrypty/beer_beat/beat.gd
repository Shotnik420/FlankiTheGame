...
@onready var klocek = preload("res://sceny/hud/beer_beat/kocek.tscn")
@onready var drinking_text = preload("res://sceny/hud/drinkingText.tscn")
...
var yellow_color : Color = Color(0.971, 0.842, 0.0, 1.0)
var blue_color : Color = Color(0.531, 0.837, 1.0, 1.0)
var gray_color : Color = Color(0.782, 0.785, 0.731, 1.0)
func _ready() -> void:
	my_scale = scale
...
func hit(pos):
	...
	took_sip.emit(sip_amount)
	spawn_new_score(best, sip_amount)
	arrows()
	tween_self()
	audio_play(sip_amount)

func audio_play(amount):
	audio.pitch_scale = randf_range(0.7,1.0)
	audio.volume_db = -7 + amount/15.0
	audio.play()
	click_audio.play()
func spawn_new_score(x, score):
	var message = "Error"
	var color = Color(0.91, 0.424, 1.0, 1.0)
	match x:
		-1: 
	...
	drink_ins.label_settings.font_color = color
...
func arrows():
	...
	arr_tween.tween_property(arr1, "position:y",-60.0,1.0)
	arr_tween.tween_property(arr2, "position:y",60.0,1.0)
	...
	arr_tween.kill()
func tween_self():
	scale = my_scale + Vector2(0.15,0.15)
	pucha.scale = pucha_scale + Vector2(0.25,0.25)
